...
def save_vocab(tokenizer, vocab_dir):
    """Saves vocabulary using UTF-8 encoding (Windows-safe)."""
...
def print_metric_per_topic(df, topics, topic_lookup, title_preprocessor, model):
    ...
    for topic in topics:
        ...
        # Handle both single value (loss only) and list [loss, accuracy]
        if isinstance(result, list) and len(result) > 1:
            accuracy = result[1] * 100
        else:
            print(f'{topic}: Model not compiled with accuracy metric')
            continue
